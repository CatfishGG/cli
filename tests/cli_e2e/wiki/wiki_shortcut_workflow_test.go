...
func TestWiki_ShortcutWorkflow(t *testing.T) {
	...
	// Setup: create an isolated parent + child via the existing bare-API
	// helpers. The shortcut tests below then operate on this known structure.
	t.Run("setup: create parent + child via bare API", func(t *testing.T) {
		parent := createWikiNode(t, parentT, ctx, "my_library", map[string]any{
			"node_type": "origin",
			"obj_type":  "docx",
			"title":     parentTitle,
		})
		spaceID = parent.Get("space_id").String()
		parentNodeToken = parent.Get("node_token").String()
	...
	// stays put (copy ≠ move). Cleanup deletes the copy.
